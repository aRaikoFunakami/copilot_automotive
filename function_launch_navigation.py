...
class LaunchNavigation(BaseTool):
    ...
    def _generate_response(self, latitude: float, longitude: float, destination: Optional[str]) -> dict:
    ...
    async def _arun(self, latitude: float = 0.0, longitude: float = 0.0, destination: Optional[str] = None):
        ...
        try:
            ...
            if not destination and (latitude == 0.0 and longitude == 0.0):
                raise ValueError("Either destination or latitude/longitude must be provided.")
